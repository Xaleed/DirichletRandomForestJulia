# Example application of Distributed Dirichlet Forest
using Distributions
using Random
using Statistics
using DataFrames
...
# Include your main Distributed Dirichlet Forest code
# include("dirichlet_forest_ml_distributed.jl")  # Uncomment this line to load your main code
...
# Load the distributed implementation
#include("C:\\Users\\Khaled\\Documents\\GitHub\\DirichletRandomForestJulia\\src\\dirichlet_forest_ml_distributed.jl")
include("C:\\Users\\29827094\\Documents\\GitHub\\DirichletRandomForestJulia\\src\\dirichlet_forest_ml_distributed.jl")

println("Number of worker processes: ", nworkers())
println("Total processes: ", nprocs())

# Create and fit the distributed forest
forest = DirichletForest(100)  # 100 trees

println("Training distributed Dirichlet Forest...")
@time begin
...
@time Y_pred = predict_dirichlet_forest(forest, X_test)
...
println("\n" * "="^50)
println("DISTRIBUTED PROCESSING INFO")
println("="^50)
println("Workers used: ", nworkers())
println("Total CPU threads available: ", Sys.CPU_THREADS)
println("Trees per worker: ", round(forest.n_trees / nworkers(), digits=2))

# Clean up worker processes when done
println("\nCleaning up worker processes...")
cleanup_workers()
println("Worker processes cleaned up.")
